...
ARCH=
VERSION=
RELEASES_URL="https://github.com/datagram1/plandex/releases/download"
...
# Set Version - Get latest release from our repository
if [[ -z "${PLANDEX_VERSION}" ]]; then
  # Get the latest release tag from our GitHub repository
  VERSION=$(curl -sL https://api.github.com/repos/datagram1/plandex/releases/latest | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/' | sed 's/^cli\/v//')
  if [[ -z "$VERSION" ]]; then
    echo "❌ No releases found in datagram1/plandex repository"
    echo ""
    echo "To install from this repository, you need to:"
    echo "1. Create a release with the plandex binary"
    echo "2. Or set PLANDEX_VERSION environment variable"
    echo ""
    echo "Example: PLANDEX_VERSION=1.0.0 curl -sL https://raw.githubusercontent.com/datagram1/plandex/main/install.sh | bash"
    echo ""
    echo "For now, falling back to original plandex.ai installation..."
...
welcome_plandex () {
  ...
  echo ""
  echo "🚀 Plandex v$VERSION • Quick Install (datagram1/plandex)"
  echo ""
  echo "$(printf '%*s' "$(tput cols)" '' | tr ' ' -)"
  echo ""
}
...
download_plandex () {
  ...
  echo "📥 Downloading Plandex tarball from datagram1/plandex"
  echo ""
  echo "👉 $url"
  echo ""
